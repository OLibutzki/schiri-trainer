...
import { starteUmgebung, oeffne, masse, MOBIL } from './umgebung.mjs';
...
test(
  'gibt keine Pruefungsantwort ohne Auswahl ab',
  { todo: 'Issue #27' },
  async () => {
    const seite = await oeffne(umgebung.browser, umgebung.adresse, { ansicht: '/pruefung' });
    await starte(seite);
...
test(
  'trennt „Pruefung abbrechen" von „Antwort abgeben"',
  { todo: 'Issue #28' },
  async () => {
...
test(
  'haelt die Beruehrziel-Groesse in der Pruefungseinrichtung ein',
  { todo: 'Issue #29' },
  async () => {
    const seite = await oeffne(umgebung.browser, umgebung.adresse, { ansicht: '/pruefung', geraet: MOBIL });
...
test(
  'zeigt das Pruefungsergebnis auch nach einem Neuladen',
  { todo: 'Issue #30' },
  async () => {
    const seite = await oeffne(umgebung.browser, umgebung.adresse, { ansicht: '/pruefung' });
    await starte(seite);
